...
int8_t ADC0_Initialize(void)
{
    ...
    //FREERUN disabled; LEFTADJ enabled; SAMPNUM 16 samples accumulation; 
    ADC0.CTRLF = 0x14;
    ...
    //ADCPGASAMPDUR 6 ADC cycles; GAIN 16X Gain; PGABIASSEL 1x BIAS current; PGAEN enabled; 
    ADC0.PGACTRL = 0x81;

    //DBGRUN disabled; 
    ADC0.DBGCTRL = 0x0;

    //DIFF enabled; MODE BURST_SCALING; START Stop an ongoing conversion; 
    ADC0.COMMAND = 0xD0;

    //RESOVR disabled; RESRDY disabled; SAMPOVR disabled; SAMPRDY disabled; TRIGOVR disabled; WCMP disabled; 
    ADC0.INTCTRL = 0x0;

    //MUXPOS ADC input pin 6; VIA Via PGA; 
    ADC0.MUXPOS = 0x46;

    //MUXNEG ADC input pin 7; VIA Via PGA; 
    ADC0.MUXNEG = 0x47;
